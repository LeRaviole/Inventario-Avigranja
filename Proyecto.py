...
def inventarioAdmin():
    ...
    tabla.grid(row=1, column=0, columnspan=3, padx=0, pady=10, ipadx=0, ipady=0, sticky=EW)
    tabla['show'] = 'headings'
...
def agregarCarro():
    def alCarro(id, cantidad):
        # Creando el carro de compras
        query = 'INSERT INTO carrito (idProducto, Nombre, Precio, Cantidad) SELECT idProducto, Nombre, Precio, Cantidad FROM Productos WHERE idProducto = ?'
        parametros = (idInput.get(), )
        run_query(query, parametros)
        query2 = 'UPDATE carrito SET Cantidad = ?'
        parametros2 = (cantidadInput.get(), )
        run_query(query2, parametros2)
        query3 = 'UPDATE carrito SET Total = Cantidad*Precio'
        run_query(query3, )
        messagebox.showinfo(title="Exito!", message="Los productos fueron agregados con exito.")
    ...
    cantidadInput = Spinbox(f6, from_=1, to=100, width=10)
...
def checkout():
    def pago():
        ...
        run_query(query, )
        messagebox.showinfo(title="Compra realizada", message="La compra fue realizada, muchas gracias!")
        quit()
    ...
    def consultar_carrito(tabla):
        ...
        for elementos in records:
            tabla.delete(elementos)
        query = 'SELECT * FROM carrito ORDER BY idProducto DESC'
        db_filas = run_query(query)
        for fila in db_filas:
            tabla.insert('', 0, values=(fila[1], fila[2], fila[3], fila[4]))
        
    f7 = Toplevel()
    root.title("Finalizar compra")

    tabla = ttk.Treeview(f7, height=10, columns=('#1', '#2', '#3', '#4'))
    tabla.grid(row=0, column=0, padx=0, pady=0, ipadx=0, ipady=0, sticky=EW)
    tabla['show'] = 'headings'
    ...
    tabla.heading('#4', text='Subtotal', anchor=CENTER)
    consultar_carrito(tabla)

    subtotales = []

    Label(f7, text="Su total es de:", font=('Arial', 15, tkFont.BOLD)).place(x=190, y=230)
    for item in tabla.get_children():
        subTotal = float(tabla.set(item, '#4'))
        subtotales.append(subTotal)

    total = sum(subtotales)
    Entry(f7, textvariable=StringVar(f7, value=total), state='readonly', bg='white').grid(row=1, column=0, padx=5, pady=10)
    Button(f7, text="Pagar", bg='#62DD6E', fg='white', font=('Arial', 10, tkFont.BOLD), command=lambda:[pago()]).grid(row=2, column=0, padx=5, pady=10)
